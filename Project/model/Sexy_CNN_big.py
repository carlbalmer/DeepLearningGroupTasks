...
class Sexy_CNN_big(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        """
        super(Sexy_CNN_big, self).__init__()
        ...
        self.alpha1 = nn.Parameter(torch.ones(1, 1))
        ...
        self.alpha2 = nn.Parameter(torch.ones(1, 1))
        ...
        self.alpha3 = nn.Parameter(torch.ones(1, 1))
    ...
    def forward(self, x):
        ...
        """
        x = self.conv1(x)
        x = self.pool1(self.alpha1.mul(x).exp()).log().mul(self.alpha1.pow(-1))
        x = self.conv2(x)
        x = self.pool2(self.alpha2.mul(x).exp()).log().mul(self.alpha2.pow(-1))
        x = self.conv3(x)
        x = self.conv4(x)
        x = self.conv5(x)
        x = self.pool3(self.alpha3.mul(x).exp()).log().mul(self.alpha3.pow(-1))
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        return x
