...
from dataset.DOGS import DogsDataset
...
def main():
    ...
    model_expected_input_size = (256, 256)
    logging.info('Model {} expects input size of {}'.format(args.model,
                                                            model_expected_input_size))

    transformations = transforms.Compose([transforms.RandomSizedCrop(256), transforms.ToTensor(),
                                          transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
                                          ])

    dog_dataset = DogsDataset('data/datasets/dogBreeds/labels.csv', 'data/datasets/dogBreeds/images/', transform=transformations)
    ...
    train_index, test_index = next(test_straf.split(dog_dataset.X_train, dog_dataset.y_train))

    train_loader = DataLoader(dog_dataset,
                              batch_size=args.batch_size,
                              sampler=SubsetRandomSampler(train_index),
                              num_workers=args.workers,
                              pin_memory=True)

    test_loader = DataLoader(dog_dataset,
                              batch_size=args.batch_size,
    ...
    model = models.__dict__[args.model](120)
